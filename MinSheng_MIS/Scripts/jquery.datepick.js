/* http://keith-wood.name/datepick.html
   Datepicker for jq 3.7.4.
   Written by Marc Grabanski (m@marcgrabanski.com) and
              Keith Wood (kbwood{at}iinet.com.au).
   Dual licensed under the GPL (http://dev.jq.com/browser/trunk/jq/GPL-LICENSE.txt) and 
   MIT (http://dev.jq.com/browser/trunk/jq/MIT-LICENSE.txt) licenses. 
...
function Datepick() {
	...
	this._defaults = { // Global defaults for all the date picker instances
		...
		showAnim: 'show', // Name of jq animation for popup
...
$.extend(Datepick.prototype, {
	...
	/* Attach the date picker to a jq selection.
	...
	/* Create a new instance object.
	   @param  target  (jq) the target input field or division or span
	   @param  inline  (boolean) true if this datepicker appears inline */
	_newInst: function(target, inline) {
		var id = target[0].id.replace(/([^A-Za-z0-9_])/g, '\\\\$1'); // Escape jq meta chars
	...
	/* Enable the date picker to a jq selection.
	   @param  target  (element) the target input field or division or span */
	...
	/* Disable the date picker to a jq selection.
	   @param  target  (element) the target input field or division or span */
	...
	/* Is the first field in a jq collection disabled as a datepicker?
	   @param  target  (element) the target input field or division or span
	...
	/* Retrieve the instance data for the target control.
	   ...
	   @throws  error if a jq problem getting data */
	...
	/* Set the dates for a jq selection.
	   @param  target   (element) the target input field or division or span
	...
	/* Get the date(s) for the first entry in a jq selection.
	   @param  target  (element) the target input field or division or span
	...
	/* Retrieve the size of left and top borders for an element.
	   @param  elem  (jq object) the element of interest
	...
	_generateHTML: function(inst) {
		...
		var clear = (this._get(inst, 'mandatory') ? '' :
			...
			'onclick="jq.datepick._clearDate(\'#' + inst.id + '\');"' +
			this._addStatus(useTR, showStatus, inst.id, this._get(inst, 'clearStatus'), initStatus) +
			'>' + this._get(inst, 'clearText') + '</a></div>');
		//var controls = '<div class="' + this._controlClass[useTR] + '">' + (isRTL ? '' : clear) +
		//	'<div class="' + this._closeClass[useTR] + '"><a href="javascript:void(0)" ' +
		//	'onclick="jq.datepick._hideDatepick();"' +
		...
		var prev = '<div class="' + this._prevClass[useTR] + '">' +
			...
			(showBigPrevNext ? '<a href="javascript:void(0)" onclick="jq.datepick._adjustDate(\'#' +
			inst.id + '\', -' + stepBigMonths + ', \'M\');"' +
			this._addStatus(useTR, showStatus, inst.id, this._get(inst, 'prevBigStatus'), initStatus) +
			'>' + prevBigText + '</a>' : '') +
			'<a href="javascript:void(0)" onclick="jq.datepick._adjustDate(\'#' +
		...
		var html = /*(closeAtTop && !inst.inline ? controls : '') +*/
			...
			'<a href="javascript:void(0)" onclick="jq.datepick._gotoToday(\'#' + inst.id + '\');"' +
			...
			for (var col = 0; col < numMonths[1]; col++) {
				...
				for (var dow = 0; dow < 7; dow++) { // Days of the week
					...
					html += '<th' + ((dow + firstDay + 6) % 7 < 5 ? '' :
						...
						'<a href="javascript:void(0)" onclick="jq.datepick._changeFirstDay(\'#' +
						inst.id + '\', ' + day + ');"' +
				...
				for (var dRow = 0; dRow < numRows; dRow++) { // Create datepicker rows
					...
					for (var dow = 0; dow < 7; dow++) { // Create datepicker days
						...
						html += '<td class="' + this._dayClass[useTR] +
							...
							(unselectable ? '' : ' onmouseover="' + 'jq.datepick._doMouseOver(this,\'' +
							inst.id + '\',' + printDate.getTime() + ')"' +
							' onmouseout="jq.datepick._doMouseOut(this,\'' + inst.id + '\')"' +
							' onclick="jq.datepick._selectDay(this,\'#' + // Select
							inst.id + '\',' + printDate.getTime() + ')"') + '>' +
	...
	_generateMonthYearHeader: function(inst, drawMonth, drawYear, minDate, maxDate,
		...
		if (secondary || !changeYear)
			...
			html += '<select class="' + this._yearSelectClass[useTR] + '" ' +
				'onchange="jq.datepick._selectMonthYear(\'#' + inst.id + '\', this, \'Y\');" ' +
				'onclick="jq.datepick._clickMonthYear(\'#' + inst.id + '\');"' +
		...
		if (secondary || !changeMonth)
			...
			monthHtml += '<select class="' + this._monthSelectClass[useTR] + '" ' +
				'onchange="jq.datepick._selectMonthYear(\'#' + inst.id + '\', this, \'M\');" ' +
				'onclick="jq.datepick._clickMonthYear(\'#' + inst.id + '\');"' +
	...
	_addStatus: function(useTR, showStatus, id, text, initStatus) {
		return (showStatus ? ' onmouseover="jq(\'#' + this._statusId[useTR] + id +
			'\').html(\'' + (text || initStatus) + '\');" ' +
			'onmouseout="jq(\'#' + this._statusId[useTR] + id +
			'\').html(\'' + initStatus + '\');"' : '');
	},
